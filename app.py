...
def index():
    ...
    trace2 = Scatter(
        x=df["Date"], y=df["DutyP"], name="Duty rate(pence/litre)", fill="tozeroy"
    )

    trace3 = Scatter(x=df["Date"], y=df["inflation"], name="Inflation", yaxis="y2")
...
def date():
    with open("data_clean.csv", "r") as inputfile:
        ...
        with open("data_date.csv", "w") as outputfile:
            ...
            for row in rdr:
                try:
                    ...
                    wtr.writerow(
                        (date_format, row[1], row[2], row[3], row[4], row[5], row[6])
                    )
        ...
        data = pd.read_csv("data_date.csv", sep=",")
